...
"! Flight Carrier -
"! A factory logic ensures that there can only be one instance for each carrier ID. CLASS lcl_carrier
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD get_instance.
    ...
    IF sy-subrc <> 0.
        RAISE EXCEPTION TYPE zcx_ss_failed
            EXPORTING
                textid = zcx_ss_failed=>carrier_not_found
                carrier_id = i_carrier_id.
...
*          value = CONV #( i_carrier_id ).
    ...
    IF sy-subrc <> 0.
        RAISE EXCEPTION TYPE zcx_ss_failed
            EXPORTING
                textId = zcx_ss_failed=>carrier_no_read_auth
                carrier_id = i_carrier_id.
